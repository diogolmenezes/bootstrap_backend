    ...
    proccess(req, res, next) {
        ...
        if (this.config.origin) {
            if (!this.hasApplication(req))
                return next(this.applicationErrors.throw('O header x-origin-application é obrigatório.', 'PreconditionFailedError')); // 412

            if (!this.hasChannel(req))
                return next(this.applicationErrors.throw('O header x-origin-channel é obrigatório.', 'PreconditionFailedError')); // 412

            if (!this.hasDevice(req))
                return next(this.applicationErrors.throw('O header x-origin-device é obrigatório.', 'PreconditionFailedError')); // 412
        }
